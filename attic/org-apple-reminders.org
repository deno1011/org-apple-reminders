...
- *Multi-file* — a reminder may be linked from any org file.  New Apple items
  ...
  linked headings they already contain.  During org -> Apple sync, the managed
  sync file uses level-1 headings as Apple list sections and level-2 headings
  as reminders.  Level-3 subtask sync is not implemented, so level-3 headings
  are never flattened into ordinary Apple reminders.
...
- =org-apple-reminders-extra-files= — extra org files scanned for existing
  linked reminders.  They may host reminders anywhere, but only the sync file
  creates Apple lists and auto-created reminders from outline structure.
  Apple subtask sync is not currently implemented; level-3 headings in the
  sync file are not auto-created as flattened reminders.
...
populated by the last pull or sync, and pushes only what differs.  In the
sync file, level-1 headings are list sections and are ensured as Apple lists.
Level-2 headings under those sections may be created as Apple reminders, with
or without an Org TODO keyword.
Extra files and agenda files remain host/update files: linked reminders keep
syncing there, and unlinked headings are created only when they carry an
explicit REMINDER_LIST property.  Level-3 sync-file headings are not flattened
into Apple reminders; true Apple subtask sync is not implemented.  New-item
creation is synchronous so the returned id can be stamped back.
...
notes from Apple; org-wins pushes the org fields if they differ.  Open items
missing from Apple become DONE in org; DONE items missing from Apple are
removed from org; CANCELLED items are deleted from Apple and removed locally.
Crucially, =REMINDER_APPLE_MOD= is stamped
...
under their =* ListName= heading, deletes completed Apple items whose DONE org
copy was removed, and refreshes the =[N/M]= progress cookies.
