#+TITLE: org-apple-reminders — Layered Source (rebuild in progress)
#+PROPERTY: header-args:emacs-lisp :tangle yes :lexical t

This is the *literate source* of the package, being rebuilt top-down into the
layered architecture described below.  The previous (working) single-file
implementation is preserved, inactive, under [[file:attic/org-apple-reminders.org][=attic/org-apple-reminders.org=]] for
reference while the rebuild proceeds.

*Build status:* COMPLETE.  All six layers are authored (L1 config → L6 business
logic + surface), and the de-duplication pass is done: the conflict decision
(=--conflict-direction=), the pull-new step (=--pull-item-into-sync-file=), the
progress-cookie pass, and the Apple-API JSON/date helpers each now live in one
place.  The tangle byte-compiles with zero warnings and passes the full =tests/=
ert suite (31/31), reproducing the attic implementation's behaviour; a live
create/update/clear-due/complete/delete round-trip against real Reminders was
verified separately.

* Architecture

=org-apple-reminders= is one literate =.org= tangling to one =.el=.  The code is
organised as a strict layered stack.  Each layer is a contiguous run of
top-level sections; a function's layer is given by the section it lives under.

#+begin_example
L6  Business logic    public commands + orchestration; reads like pseudo-code.
                      org-apple-reminders-sync / --background-pull / --on-save /
                      -setup.  No JXA, no buffer string-mangling — only calls
                      named steps from L5 and below.
L5  Org I/O           read/write org buffers & property drawers, multi-file
                      routing, the per-file reconcile/pull steps the
                      orchestrators name.
L4  Model & mapping   the snapshot + counts data shapes, org<->Apple field
                      translation, the conflict-direction decision.  Mostly
                      pure; a few field readers/writers touch the entry at point.
L3  Apple API (JXA)   typed wrappers: create / update / complete / delete /
                      fetch reminders and lists.  Knows Apple's shape, nothing
                      about org.
L2  Transport         osascript sync/async only; strings in, JSON/strings out.
L1  Config & state    defcustom, internal defvars, the included-list filter.
#+end_example

*The rule: a layer may only call downward.*  L6 calls L5/L4; L5 calls L4/L3;
L4 is pure; L3 calls L2; L2 calls nothing in this package.  A call that points
upward means the abstraction is wrong — stop and reconsider.  In a single file
the layer is visible from the *section heading*, the way the filename carried
it in the bootstrap subsystem this design borrows from.

Lower layers are dumb and I/O-bound; upper layers do no string-mangling.  This
is the same spine as a provisioning bootstrap: primitives at the bottom, an
orchestrator at the top that ensures-A-then-B-then-C.
...
The customisation group, the user-facing =defcustom= options and faces, and the
internal state vars (=--cache=, =--syncing=, =--sync-timer=, index caches).
Everything above this layer reads these; nothing here depends on anything else.
...
The osascript boundary: run a JXA script synchronously for its result, or
asynchronously (fire-and-forget, used by pushes and the background pull so Emacs
never blocks).  Strings in, strings out — no Apple or org semantics here.
...
Typed wrappers around JXA scripts: the fetch-everything script, and
create / update / complete / delete reminders, ensure / delete lists, and the
list-name query.  Each knows Apple's data shape and composes L2 transport; none
of it knows anything about org buffers.
...
The model layer holds the run's *data shapes* and the pure org↔Apple field
translation.  It depends only on the Apple-API and transport layers below.  Most of it is
pure (Apple-data accessors, field comparison, the conflict decision); a handful
of field readers/writers act on the org entry at point.

It defines the two structs the orchestrators thread through their steps, the
list filter, the org↔Apple field translation, and the conflict-resolution
helpers (mod-date comparison, field diffing, apply/backfill).  The per-heading
conflict =cond= inside L5's =--sync-reconcile-file= will be lifted into a
=--conflict-direction= decision here once the package is whole and the test
suite is green.

- =--counts= is the mutable per-run tally, replacing the loose =n-done= /
  =n-pushed= / … =let= bindings the legacy orchestrator carried.
- =--snapshot= captures one fetch of Apple state: the raw list data, an id→item
  hash, the current Apple list names/ids, the org-side id→file index, and the
  list names seen on the previous run.  =--fetch-snapshot= builds one
  synchronously; =--snapshot-from-json= builds one from an async fetch's JSON.
...
* L5 — Org I/O: file & heading helpers

The org-side workhorses the sync steps build on: multi-file discovery and the
=--with-known-files-cache= macro, list-section navigation and stamping, heading
readers, the reminder-heading insert/relocate/delete primitives, and the
delete-mark / cancellation finalisers.  All are extracted verbatim from =attic/=
(sections "Multi-file helpers", "Org file helpers", and the private helpers of
"Org heading → Apple push"); the public push/delete *commands* that wrap them
stay for the L6 surface.
...
* L5 — Org I/O: sync orchestration steps

These are the named steps the L6 orchestrators call.  Each wraps a contiguous
slice of the legacy =org-apple-reminders-sync= / =--background-pull= /
=--push-to-apple= (preserved verbatim in =attic/=); the only change is the glue:
free counters become fields of the =--counts= struct, and the shared fetch state
becomes =--snapshot= accessors.  The per-heading conflict resolution inside
=--sync-reconcile-file= is unchanged and will be lifted into an L4
=--conflict-direction= once the model layer is complete and the test suite is
green again.
...
* L6 — Surface: commands, hooks & integration

The user-facing layer: the interactive commands (push, mark/delete, list
management, migration), the live-edit hooks and advice, org-capture and
org-agenda integration, the delete-mark display overlays, the keymap, and the
small leaf utilities the orchestrators lean on (buffer autosave, JXA drain,
load diagnostics).  Extracted verbatim from =attic/=.
...
* L6 — Business logic: orchestration & entry points

This is the top of the stack.  Every function here reads as a sequence of named
steps; the steps themselves live in L5 (org I/O) and below.  Nothing here builds
a JXA script, parses JSON, or pokes a property drawer directly.

The lower-layer operations this layer calls are declared in [[*Lower-layer contract][Lower-layer
contract]] at the end of the file, with a one-line meaning each.  As each layer
is built downward, its functions move out of the contract and into their own
section, and the =declare-function= for them is removed.

The run's tally (=org-apple-reminders--counts=) and the fetched-state model
(=org-apple-reminders--snapshot=) are data shapes owned by the model layer (L4);
this layer only creates a counts, threads it through the steps, and reads it back
for the final report.

** Full bidirectional sync

The orchestrator below is the whole of =org-apple-reminders-sync= expressed as
named steps.  Each step maps onto a contiguous slice of the legacy 285-line
function (preserved in =attic/=); building L5 downward fills these in without
changing the order or the behaviour:

- =--snapshot= fetches Apple state (blocking) and returns the run's model: the
  raw list data, an id→item hash, the current Apple list names/ids, the org-side
  id→file index, and the list names seen on the previous run.
- =--sync-reconcile-list-sections= reconciles the sync file's =* List= sections
  against Apple's lists (drop sections for lists Apple deleted, mark the rest
  DONE).
- =--sync-delete-marked= deletes reminders flagged =REMINDER_DELETE= in any file.
- =--sync-reconcile-file= is phase 1: per linked heading in a known file, decide
  via the conflict direction (apple-wins / org-wins / backfill / done / create /
  delete / cancel) and apply it.
- =--sync-pull-new= is phase 2: prune excluded lists, ensure local list
  sections exist in Apple, and pull Apple items not yet linked into the sync
  file.

Before fetching, modified known-file buffers are auto-saved (firing the
=after-save-hook= → async push) and the run waits for in-flight JXA to drain, so
the fetch sees Apple's post-push state and the org-changed signal is reliable.
...
The periodic timer and the post-setup idle pull both call this.  It is the
non-destructive sibling of the full sync: it fetches asynchronously, refreshes
the agenda mirror, then per known file marks vanished/ completed items DONE and
applies Apple-changed field values — it never creates, deletes, prunes, or
pushes.  New open Apple items are pulled into the sync file only.
...
** Save-path push (org → Apple)

Called from the save hook for any known file.  Pushes changed entries, creating
new ones synchronously (so the Apple id can be stamped back) and updating
existing ones asynchronously.  Auto-creation only happens for sync-file level-2
list members or explicit =REMINDER_LIST= headings.
...
* De-duplication pass (done)

After the faithful layered extraction, the duplicated logic the layering exposed
was collapsed, each step verified by re-running the ert suite and keeping the
public surface and stubbed L3 boundary intact:

1. *Conflict decision* — the per-heading dual-timestamp =cond= became the pure
   L4 =--conflict-direction= (=:apple-wins= / =:apple-reconciled= / =:org-wins= /
   =:backfill=), shared by =--sync-reconcile-file= and the background pull.
2. *Pull-new* — the sync and background-pull both insert new Apple items via the
   one =--pull-item-into-sync-file= helper.
3. *Progress cookies* — the repeated level-1 =[/]= pass became
   =--ensure-list-cookies=; the sync-file preamble became
   =--sync-file-template=.
4. *Apple API boilerplate (L3)* — the repeated JSON-parse dance became
   =--parse-json= / =--jxa-run-json=, and the =new Date(...)= due expression
   became =--due-date->js=.  No JXA dispatch table: each operation's JavaScript
   is genuinely distinct, so only the elisp-side boilerplate was shared (the
   generated osascript is byte-for-byte unchanged).

This was de-duplication, not line-golf: the file stayed near its original size,
but each concern now has a single source of truth.
