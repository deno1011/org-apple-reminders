...
org file — and creates a matching Apple reminder, stamping =REMINDER_ID= and
=REMINDER_LIST= back onto the heading and registering the file so future syncs
keep it updated.  It also clears any =REMINDER_NOSYNC= marker.
...
=* ListName= top-level heading for a list, and inserting a freshly pulled
Apple item as a =** TODO= heading underneath it with all its properties.
