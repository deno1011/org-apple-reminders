...
closures in this file, which is required for the lambda callbacks used in async
JXA calls — without lexical binding, closures that capture local variables
(such as markers for =REMINDER_ORG_MOD= stamping) would not work correctly.
...
entirely; you can still trigger pulls manually with =C-c r R= (full sync).
...
=org-apple-reminders-included-lists= is described in the List Filter section
below.
...
Having a single predicate rather than inline checks at each call site means the
filtering logic can be changed or extended in one place.
...
second sync in response to intermediate buffer writes.  Without this guard,
for example, marking an item DONE during sync would trigger the save hook
which would push that completion back to Apple, which is already reflected.
...
values and only pushes entries that differ.  This keeps saves instant and
non-blocking.

=org-apple-reminders--sync-timer= holds the handle for the periodic background
pull timer, so that =org-apple-reminders-setup= can avoid starting a duplicate
timer if called more than once.
...
The package never uses AppleScript directly — JXA allows us to write
structured code, use JSON for data exchange, and batch property fetches for
performance.
...
All writes (completing, creating, updating reminders) and all background pulls
use this async form.  The only synchronous Apple calls are the full fetch in
=org-apple-reminders-sync= and the new-item create/update in that same
function, where we need the returned modificationDate or new ID before we can
proceed.
...
Apple for its first list name.  This call is intentionally lazy — we do not
ask Apple at load time, only when actually needed.
...
list of strings by stripping the JSON punctuation characters.

=org-apple-reminders-create-list= creates a new Apple Reminders list
asynchronously by constructing a new =List= object in JXA and pushing it onto
=app.lists=.

=org-apple-reminders-open-file= opens =org-apple-reminders-sync-file= directly
in the current window — the everyday entry point for editing reminders.
...
comparison.  They run entirely in Emacs and make no JXA calls.
...
from =org-log-done= are not sent to Apple Reminders as notes.
...
insertion into headings pulled from Apple.
...
shifting the displayed day depending on the user's timezone.
...
=getMonth()+1=, and =getDate()= from the local =Date= object and format them
as =YYYY-MM-DD= ourselves.
...
consistency is what matters there, not local date rendering.
...
Apple via =reminder.delete()=.  Always async; an optional callback lets the
caller know when the delete has finished.
...
snapshots all IDs before the push, pushes the new item, snapshots again, and
returns the ID that appeared in the second snapshot but not the first.  This
runs synchronously because the caller needs the new ID to stamp =REMINDER_ID=
back onto the org heading.
...
would be a no-op in JXA).  Without a =callback= argument it runs synchronously
and returns Apple's =modificationDate= immediately after the push — this
timestamp is stored as =REMINDER_ORG_MOD= to record that org was the last
writer.  With a callback it runs async; the callback receives the
modificationDate string and stamps =REMINDER_ORG_MOD= itself.
...
can produce false conflicts (e.g. whitespace differences).  This package
instead tracks two Apple =modificationDate= timestamps per org entry.
...
interaction → Apple wins: pull priority/due/flagged from Apple, stamp
=REMINDER_APPLE_MOD = A=.  Otherwise → org wins: compare org fields against
Apple's fetched state; push only if different, stamp =REMINDER_ORG_MOD= with
Apple's post-push modificationDate.
...
set (first sync of this entry — treat as if Apple has always been up to date,
so org wins by default).
...
be chosen interactively with completion from the user's actual Apple lists.
The new reminder will appear in the org file on the next background pull or
full sync.
...
back onto the org heading so future syncs can track it bidirectionally.

With a prefix argument (=C-u C-c r p=), it prompts for the target list name
with completion.  Without a prefix argument it uses the default list.
...
2. Fires an async JXA =delete()= call to remove the reminder from Apple.
3. Removes the item from the in-memory cache so push-on-save won't try to
   update a reminder that no longer exists.
...
fires immediately via =org-after-todo-state-change-hook=.  =DONE= or =CANCELLED=
→ sets =completed=true= in Apple.  Any active state (=TODO=, =NEXT=,
=WAITING=) → sets =completed=false= (reopens) in Apple.  Both are async so
Emacs does not freeze.
...
completed in Apple), and we must not push those back to Apple a second time.
...
next sync knows org was the last writer and does not overwrite the change with
stale Apple data.

Using advice rather than hooks is necessary here because these org commands
do not have dedicated hooks that fire after the modification is complete and
point is on the affected heading.
...
Notes are indented with three spaces so they read well under the heading.
...
during the map pass.  New items are processed after the scan pass so that
setting =REMINDER_ID= / =REMINDER_LIST= properties on them does not interfere
with the =org-map-entries= iteration.
...
Unlike the background pull (which only reads Apple → org) or the save hook
(which only writes org → Apple), the full sync handles all six cases that can
arise.
...
1. *Fetch* — run the JXA fetch script synchronously.  We need all of Apple's
   current data before we can make any decisions.

2. *Scan pass* — visit every org entry, classify into buckets:
   ...
   - Immediate synchronous push — org-wins branch (inside the scan)

3. *Apply pass* — process each bucket using stored markers.

4. *Pull pass* — insert any Apple reminders not yet in the org file.

5. *Stamp & save* — update =REMINDER_APPLE_MOD= for all synced entries,
   update =[N/M]= cookies on list headings, save the buffer, reveal changed
   headings.

The entire operation runs inside =org-save-outline-visibility= so heading
fold states are preserved across the sync.
...
1. Updates the in-memory cache so the save hook has fresh data for comparison.
2. Writes the optional agenda file.
3. If the sync file exists, opens it and applies three categories of change:
   - Mark DONE any items Apple completed or deleted.
...
The background pull is deliberately conservative: it uses the same
two-timestamp conflict detection as the full sync, so it will not stomp on
org changes that have not been pushed yet.
...
Setting =org-apple-reminders-auto-sync-interval= to =0= skips the timer
entirely; users who want manual-only sync can do so.
...
=REMINDER_LIST= properties of the heading at point and returns them as a
=(list-name . id)= cons cell.  This is used by delete and push-heading to
identify which Apple reminder to operate on.  It returns =nil= if the heading
has no =REMINDER_ID=, making it safe to call from any org heading.

=org-apple-reminders--write-agenda-file= is called by the background pull to
keep the optional agenda file up to date.  When =org-apple-reminders-agenda-file=
is set, this function writes a clean org file with all open reminders
formatted for org-agenda — no =REMINDER_APPLE_MOD= / =REMINDER_ORG_MOD=
property drawers, just =REMINDER_LIST= and =REMINDER_ID= for identification.
The file is also added to =org-agenda-files= so agenda commands pick it up.
...
does not complain about a missing file before the first sync.  The function is
also registered on =org-agenda-mode-hook= so agenda buffers that open before
the first sync see the file correctly.
...
order (to preserve buffer positions during deletion), then re-inserts them
under their correct list headings with their heading level incremented by one.
