...
Reminders and the org sync file in one operation.  It works from both the
=*Apple Reminders*= dashboard (press =d=) and directly in =reminders.org= when
point is on a reminder heading.
