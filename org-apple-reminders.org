...
removes the heading from the org file.  =org-apple-reminders-remove-from-apple=
...
in the org file as an ordinary TODO and is never pushed back.  Both act on the
heading at point, or — with an active region — on every reminder in the
selection at once, behind a single confirmation prompt.  The shared helpers
