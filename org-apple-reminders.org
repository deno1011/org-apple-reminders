...
- *Multi-file* — a reminder may be linked from any org file.  New Apple items
  ...
  linked headings they already contain.  During org -> Apple sync, known
  reminder files use plain level-1 headings as Apple list sections; TODOs
  without a list section fall back to the configured/default Apple list.
...
- =org-apple-reminders-extra-files= — extra org files whose headings define
  Apple Reminders structure.  Agenda files are still scanned for existing
  linked reminders, but only the sync file and extra files create Apple lists
  from level-1 headings.
...
and then marks the matching =* ListName= section DONE in the sync file.  A
DONE list section is the marker that the list was intentionally removed and
must not be recreated by a later sync.  Synced list sections carry
=REMINDER_LIST_SYNCED=, =REMINDER_LIST_NAME=, and optionally
=REMINDER_LIST_ID= properties so this decision survives an Emacs restart.
...
comparison works.  For lists, the script also returns the name and, when Apple
exposes one through JXA, a stable list id; list modification timestamps are not
available through this path, so the package persists sync markers on the Org
...
populated by the last pull or sync, and pushes only what differs.  In every
the sync file and `org-apple-reminders-extra-files', plain level-1 headings
are list sections and are ensured as Apple lists even when they have no child
tasks.  Level-1 TODO headings with children are list sections too; bare
top-level TODOs in those files fall back to the configured/default Apple list.
Agenda files remain update-only unless a heading carries an explicit
REMINDER_LIST.  New-item creation is synchronous so the returned id can be
stamped back.
