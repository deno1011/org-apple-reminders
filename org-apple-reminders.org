...
=--top-level-todo-warnings= de-duplicates warnings for sync-file list names
that look like reminders but are written at list-section level.
...
Top-level headings in the sync file are list sections.  A top-level
=* TODO Call supplier= is therefore treated as a new Apple list named
="Call supplier"=, and the package shows a warning telling the user to indent
the heading as =** TODO= under an existing =* List= heading if that was the
intent.  This avoids silently assigning the task to the preceding list.
