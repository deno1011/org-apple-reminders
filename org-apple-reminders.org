...
logic + surface).  The tangle byte-compiles with zero warnings and passes the
full =tests/= ert suite (10/10), reproducing the attic implementation's
behaviour.  The structure is in place; the line-count reduction (collapsing the
duplicated JXA / field / drawer logic) is the next phase, now safe to do under
the green test suite.
...
* Next phase: line reduction

All six layers are built and the =tests/= suite is green, so the structural
rebuild is done.  This pass was a *faithful* extraction — the gnarly bodies were
copied byte-for-byte from =attic/=, so the file is still ~2790 lines.  The
readability and line-count win comes next, now safe to do under the test suite:

1. *One JXA script builder + dispatch (L2/L3).*  Collapse the repeated inline JS
   string-building in the create/update/complete/delete wrappers.
2. *Fold the per-heading conflict =cond= into an L4 =--conflict-direction=*
   returning =:push= / =:pull= / =:backfill= / =:done= / =:create= / =:delete= /
   =:cancel=.  =--sync-reconcile-file= and =--background-pull= then share one
   decision instead of each inlining the dual-timestamp comparison.
3. *One property-drawer read/write pair (L4/L5)* to remove the duplication
   between the push path and the org-file helpers.
4. *Unify the sync vs background-pull "pull-new" + cookie passes* — they are
   near-identical and can share a helper.

Each step keeps the public surface and the stubbed L3 boundary intact, and is
verified by re-running the ert suite.
