...
- =org-apple-reminders-included-lists= — restrict which Apple lists are pulled
  in; =nil= means all.  Headings already linked keep syncing regardless.
...
=org-apple-reminders--list-included-p= implements
=org-apple-reminders-included-lists=.  When the variable is =nil= every list
participates; otherwise only the named lists do.  The filter only gates
*new* Apple items — a heading already carrying a =REMINDER_ID= keeps syncing
even if its list was later removed from the variable.
