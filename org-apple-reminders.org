...
- =org-apple-reminders-extra-files= — extra org files scanned for linked
  headings, so reminders can live outside the sync file.  In every known
  reminder file, level-1 headings are Apple list sections.
...
known reminder file, plain level-1 headings are list sections and are ensured
as Apple lists even when they have no child tasks.  Level-1 TODO headings with
children are list sections too; bare top-level TODOs fall back to the
configured/default Apple list.  New-item creation is synchronous so the
