...
existing items that changed are pushed immediately (synchronous
=--update-in-apple= returns the new =modificationDate= which is stamped as
=REMINDER_ORG_MOD=); completed items trigger an async complete call.
...
If =--push-to-apple= modifies the buffer (e.g. by stamping =REMINDER_ORG_MOD=
or setting a new =REMINDER_ID=), the buffer becomes modified again.  The
=save-buffer= call at the end handles this with =--syncing= still =t= so it
does not recurse.
