...
file a level-1 heading is a list section and is ensured as an Apple list even
when it has no child tasks.  An unlinked TODO becomes a new Apple reminder only
when it is below its nearest =* List= section (unless it carries
=REMINDER_NOSYNC=).  In other org files, an unlinked TODO is created only when
it has an explicit =REMINDER_LIST= property or the current file matches
=org-apple-reminders-file-list-map=.  Unmapped files remain update-only for
already-linked headings.  New-item creation is synchronous so the returned id
can be stamped back.
