...
(defun org-apple-reminders--parse-json (raw &rest args)
  "Parse RAW as JSON, forwarding ARGS to `json-parse-string'; nil on error."
  (condition-case nil
      (apply #'json-parse-string raw args)
    (error nil)))

(defun org-apple-reminders--jxa-run-json (script &rest args)
  "Run JXA SCRIPT synchronously and parse its stdout as JSON; nil on error.
ARGS are forwarded to `json-parse-string'."
  (apply #'org-apple-reminders--parse-json
         (org-apple-reminders--jxa-run script) args))

(defun org-apple-reminders--due-date->js (due)
  "Return a JXA `new Date(...)' expression for the org DUE timestamp string."
  (format "new Date(%s)"
...
since EventKit's `lastModifiedDate' formatting is locale-dependent."
  (let* ((bare-id (org-apple-reminders--strip-id-prefix id))
         (script  (format org-apple-reminders--clear-due-template
                          (json-encode bare-id)))
         (parsed  (org-apple-reminders--jxa-run-json
                   script :object-type 'alist :null-object nil)))
    (when (alist-get 'ok parsed)
      ;; Pull a fresh ISO modDate via JXA since the EventKit one was
      ;; locale-dependent (its description format varies by user locale).
      ;; JXA's modificationDate.toISOString() is stable.
...
JSON.stringify(newId);"
           ...
           (if due (format ",dueDate:%s" (org-apple-reminders--due-date->js due)) ""))))
    (org-apple-reminders--jxa-run-json script)))
...
errors with -1700."
  (let* ((title      (alist-get 'title    vals ""))
         ...
         (due-line   (if clear-due ""
                       (format "r.dueDate=%s;" (org-apple-reminders--due-date->js due))))
         (script
          (format
...
var md=r.modificationDate();JSON.stringify((md&&md instanceof Date)?md.toISOString():null);"
           ...
           (if flagged "true" "false")
           due-line)))
    ;; When clearing, do the EventKit clear FIRST.  Clearing after the JXA
    ;; write lets EventKit's stale snapshot race the JXA-bumped modDate and
    ;; leave the old dueDate in place; clearing first means the JXA write
    ;; (with an empty `due-line') never touches dueDate, so the clear sticks.
    (when clear-due
      (org-apple-reminders--clear-due-in-apple id))
    (if callback
        (org-apple-reminders--jxa-async
         script
         (lambda (raw) (funcall callback (org-apple-reminders--parse-json raw))))
      (org-apple-reminders--jxa-run-json script))))
...
(defconst org-apple-reminders--create-recurring-template
  "ObjC.import('EventKit');
var store=$.EKEventStore.alloc.init;
...
(defun org-apple-reminders--reminder-exists-p (list-name title)
  "Non-nil if a reminder named TITLE already exists in LIST-NAME.
Uses Reminders app scripting (AppleEvents permission only — no EventKit read,
so it never triggers a separate TCC prompt or headless hang)."
  (org-apple-reminders--jxa-run-json
   (format "var l=Application('Reminders').lists.byName(%s);
JSON.stringify(l.reminders.whose({name:%s})().length>0);"
...
recurrence.  Idempotent across invocations: checks via Reminders app scripting,
then creates via EventKit only when absent.

Create-only by design — the reminder is native (Apple owns the recurrence)
and `org-apple-reminders-sync' never touches it.  Put such reminders in a list
outside the synced set.  Returns the create result alist, or nil when the
reminder already existed."
  (unless (org-apple-reminders--reminder-exists-p list-name title)
    (let ((d (decode-time first-due))
          (f (pcase freq ('daily 0) ('weekly 1) ('monthly 2) (_ 0))))
      (org-apple-reminders--jxa-run-json
       (format org-apple-reminders--create-recurring-template
               (json-encode list-name) (json-encode title)
...
(defun org-apple-reminders--conflict-direction (apple)
  "Decide how to reconcile the linked, open heading at point against APPLE item.
Compares Apple's modDate against max(REMINDER_APPLE_MOD, REMINDER_ORG_MOD) and
returns one of:
- `:apple-wins'        Apple changed and its tracked fields differ — pull them.
- `:apple-reconciled'  Apple's modDate advanced but every field already matches
                       org — just restamp REMINDER_APPLE_MOD.
- `:org-wins'          Apple not newer and REMINDER_ORG_MOD > REMINDER_APPLE_MOD
                       — push org (including cleared fields).
- `:backfill'          Neither side newer — fill missing org fields from Apple,
                       push only non-empty org diffs."
  (let* ((a-mod (org-apple-reminders--apple-item-mod-date apple))
    ...
    (if apple-changed
        (if (org-apple-reminders--field-values-differ-p
             ...
             (org-apple-reminders--current-org-field-values))
            :apple-wins
          :apple-reconciled)
      (let ((apple-mod-prop (org-entry-get nil "REMINDER_APPLE_MOD"))
            (org-mod-prop   (org-entry-get nil "REMINDER_ORG_MOD")))
        (if (and org-mod-prop apple-mod-prop
...
(defun org-apple-reminders--pull-item-into-sync-file (item entry id-index sync-file)
  "Insert open Apple ITEM under its list section (from ENTRY) in the sync file.
Stamp the list section, set REMINDER_APPLE_MOD from the item, and index its id
in ID-INDEX against SYNC-FILE.  Return a marker at the list heading so callers
that track changed positions can reveal it."
  (let ((list-name (alist-get 'list entry)))
    ...
    (let ((marker (point-marker)))
      ...
      (puthash (alist-get 'id item) sync-file id-index)
      marker)))
                      ...
                      (when (not (gethash id id-index))
                        (if (org-apple-reminders--apple-item-completed-p item)
                            (progn
                              ...
                              (setq n-deleted (1+ n-deleted)))
                          (push (org-apple-reminders--pull-item-into-sync-file
                                 item entry id-index sync-file)
                                changed-positions)
                              ...
                              (let* ((id (org-entry-get nil "REMINDER_ID"))
                                ...
                                (when (and item
                                           ...
                                           (org-apple-reminders--active-heading-state-p
                                            (org-get-todo-state)))
                                  (pcase (org-apple-reminders--conflict-direction item)
                                    (:apple-wins
                                     (push (list (point-marker)
                                                 (org-apple-reminders--apple-field-values item))
                                           field-updates))
                                    (:apple-reconciled
                                     (let ((a-mod (org-apple-reminders--apple-item-mod-date item)))
                                       (when (stringp a-mod)
                                         (org-set-property "REMINDER_APPLE_MOD" a-mod))))
                                    (_ nil)))))
                            nil nil)
                                   ...
                                   (dolist (item items)
                                     (let ((id (alist-get 'id item)))
                                       (when (and (not (gethash id id-index))
                                                  (not (org-apple-reminders--apple-item-completed-p item)))
                                         (org-apple-reminders--pull-item-into-sync-file item entry id-index sync-file))))))))
                           (when is-sync-file
                             (org-apple-reminders--ensure-list-cookies))
                           (save-buffer)))))))
