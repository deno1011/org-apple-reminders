...
- DONE in org, open in Apple → mark Apple completed.
- Open in org, completed or missing in Apple → mark DONE in org.
- DONE in org, missing in Apple → remove the org subtree.
- DONE in Apple, missing in org → delete the Apple item.
...
Apple's post-push state."
  ...
  (org-apple-reminders--with-known-files-cache
    ...
    (let ((org-apple-reminders--syncing t))
      ...
      (when (> n-deleted 0)
        (org-apple-reminders--wait-for-async-jxa))
      (message "Reminders: %d deleted  %d←DONE  %d lists→DONE  %d→Apple  %d←Apple  %d updated  %d pruned"
               n-deleted n-done n-lists-done n-pushed n-pulled n-updated n-pruned)))))
